...
const getPackageNameWithoutScope = pkg => pkg.substring(pkg.lastIndexOf(path.sep) + 1);

(function updatePeerDependencies() {
    const managedPkgs = getManagedPackages();
    const pkgFolderNames = lodash.map(managedPkgs, pkg => getPackageNameWithoutScope(pkg));
    const managedPkgsAndContent = lodash.reduce(
        pkgFolderNames,
        (pkgs, pkg) => (
            { 
                ...pkgs,
                [pkg]: readJsonSync(path.resolve(__dirname, '..', 'libs', pkg, "package.json"))
            }),
        {}
    );
    ...
    managedPkgs.forEach(pkg => {
        ...
        const peerDependencies = lodash.intersection(lodash.keys(pkgJsonContent.peerDependencies), managedPkgs);
